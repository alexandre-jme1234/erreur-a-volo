...
describe 'who is the biggest number' do
  ...
  it 'tells me the biggest' do
    expect(who_is_bigger(84, 42, nil?)).to eq(nil?)
    expect(who_is_bigger(nil?, 42, 21)).to eq(nil?)
    expect(who_is_bigger(84, 42, 21)).to eq(a)
    expect(who_is_bigger(42, 84, 21)).to eq(b)
    expect(who_is_bigger(42, 21, 84)).to eq(c)
  end
end
